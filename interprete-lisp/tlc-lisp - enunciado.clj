...
(defn igual?
  ...
  [arg1 arg2] (if (not (nil? (re-matches (re-pattern arg1) arg2)))
                true
                false
              )
)
...
(defn fnc-append
  ...
  [lista]
   (if (= (count lista) 2)
     (cond
     		 (every? true? (map #(and (list? %) (not (empty? %))) lista)) (flatten (map concat lista))
     		 
     		 (or
     		   (and
            (list? (first lista))
            (not (empty? (first lista)))
            (or
              (list? (second lista))
              (nil? (second lista))
            )
     		   ...
     		   (and
            (list? (second lista))
            (not (empty? (second lista)))
            (or
              (list? (first lista))
              (nil? (first lista))
            )
     		   )
     		 ) (flatten (filter #(and (not (empty? %)) (list? %)) lista))

     		 (or
     		   (and
            (list? (first lista))
            (not (empty? (first lista)))
            (not (list? (second lista)))
            (not (nil? (second lista)))
     		   )
     		   (and
            (list? (second lista))
            (not (empty? (second lista)))
            ...
            (not (nil? (first lista)))
     		 ...
     		 ) (list '*error* 'list 'expected (first (filter #(not (list? %)) lista)))

     )
     (controlar-aridad lista 2)
   )
)
